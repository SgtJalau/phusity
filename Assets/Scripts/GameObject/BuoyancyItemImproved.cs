...
public class BuoyancyItemImproved : MonoBehaviour
{
//	public Ocean ocean;
    ...
    private readonly List<WaterBody> _waterBodies = new List<WaterBody>();
    ...
    private void Start()
    {
        ...
        if (_collider == null)
        {
            ...
            _collider = GetComponent<Collider>();
            Debug.LogWarning(string.Format("[Buoyancy.cs] Object \"{0}\" had no collider. MeshCollider has been added.",
                name));
        }

        isMeshCollider = _meshCollider != null;
        ...
        if (_rigidbody == null)
        {
            ...
            Debug.LogWarning(string.Format("[Buoyancy.cs] Object \"{0}\" had no Rigidbody. Rigidbody has been added.",
                name));
        }
        ...
        Debug.Log(string.Format("[Buoyancy.cs] Name=\"{0}\" volume={1:0.0}, mass={2:0.0}, density={3:0.0}", name,
            volume, _rigidbody.mass, density));
    }
    ...
    private float GetWaterLevel(float x, float z)
    {
        ...
        {
            waterLevel = Math.Max(waterLevel, waterBody.GetYBound());
        }
...
//		return ocean == null ? 0.0f : ocean.GetWaterHeightAtLocation(x, z);
    ...
    private void OnTriggerExit(Collider water)
    {
        ...
        _waterBodies.Remove(colliderBody);
    }
